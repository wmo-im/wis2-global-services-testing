...
. Use hive/mqttx-cli scripting provided by Remy to perform this test

====== Evaluate

. If the Global Broker subscribes to all 200 WIS2 Node subscriptions,  the test passes.  Otherwise, the test fails.
...
. If the Global Broker supports a minimum of *1000* subscribers,  the test passes.  Otherwise, the test fails.
...
. If the Global Broker supports a minimum of *10000* messages per second,  the test passes.  Otherwise, the test fails.
