...
For the WNM:
. Once the _cache_ notification message is received by the test MQTT client (from the dev/test GC), extract the canonical link from the cached messages.
. Prepare and publish an 'ab' scenario configuration using the extracted canonical link.
. The test client should start 1000 concurrent downloads of the data object from the canonical link provided in the _cache_ WNM. This will be accomplished by distributing the concurrent downloads across the available test nodes (10) and achieving a total of 1000 concurrent downloads.
