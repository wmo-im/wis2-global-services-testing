...
LOG_FILE="${OUTPUT_DIR}/gb_tests_${DATETIME}_${GC_METRICS_REPORT_BY}.log"
XML_FILE="${OUTPUT_DIR}/gb_tests_${DATETIME}_${GC_METRICS_REPORT_BY}.xml"

# Run the pytest and capture the output
pytest gb_functional_tests.py --junitxml=$XML_FILE -l -rA | tee $LOG_FILE
